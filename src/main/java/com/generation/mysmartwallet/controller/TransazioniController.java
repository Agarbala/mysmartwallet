...
public class TransazioniController {
    ...
    @GetMapping("/elimina")
    public String eliminaTransazione(@RequestParam("id") int idTransazione, @RequestParam("pagina") String pagina,
                                     HttpSession session) {
    ...
    public String listaTransazioni(Model model, HttpSession session) {
        model.addAttribute("transazioni",
                context.getBean(Conto.class, SessionUtil.idFromSession(session)).getTransazioni());
